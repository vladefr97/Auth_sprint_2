...
from api.v1.blueprints.oauth.client import OAuthClientCredentials, OAuthClientName, OAuthClient
from api.v1.blueprints.oauth.schema.yandex import YandexUserInfoSchema
from api.v1.blueprints.oauth.utils import register_user_if_not_exists, attach_social_account_to_user
...
class YandexOAuthClient(OAuthClient):
    ...
    def get_user_info(self, request=None):
        ...
        registered_user = register_user_if_not_exists(login=user_info.login,
                                                      email=user_info.emails[0] if len(user_info.emails) else "")
        attach_social_account_to_user(social_id=user_info.id, social_name=self.credentials.provider.value,
