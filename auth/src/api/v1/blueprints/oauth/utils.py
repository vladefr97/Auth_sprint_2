...
def register_user_if_not_exists(email: str, login: str) -> User:
    ...
    if not current_user:
        ...
        current_user = User(
            login=login, email=email, password=User.generate_hash(generate_random_string()), user_role=role_model.id
        )
...
def attach_social_account_to_user(social_name: str, social_id: str, user: User) -> None:
